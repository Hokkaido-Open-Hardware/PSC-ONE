...
// Fixed INT8 4x4 virtual array: scheduler -> MUL4 -> registered WB -> ACC16.
module PSC_NPU_SystolicArray4x4 (
    ...
    // A/B shift context is unchanged. Snapshot storage lives in u_mul4.
    ...
    always @(posedge clock) begin
        if (reset_n) begin
            ...
            for (check_lane = 0; check_lane < 4; check_lane = check_lane + 1) begin
                if (wb_valid[check_lane]) begin
                    if (wb_id[check_lane*4 +: 2] !== check_lane[1:0])
                        $fatal(1, "NPU write-back lane/ID mismatch");
                    for (other_lane = check_lane + 1; other_lane < 4; other_lane = other_lane + 1)
