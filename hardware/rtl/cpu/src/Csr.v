...
module Csr (
    ...
    // ---------------- outputs ----------------
    // Expose the architectural CSR state directly.  The legacy design used
    // a second, csr_valid-gated bank here; that bank lagged CSR writes by a
    // cycle and could hide newly enabled timer interrupts from the core.
    always @(*) begin
    ...
    // Status inputs are sampled independently of CSR commit timing.
    ...
    always @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
            ...
            // Machine interrupts are accepted outside the CSR instruction
            // commit phase, so trap entry must not be gated by csr_enb.
            if (set_mtrap) begin
                csr_priv_mode       <= PRIV_M;
