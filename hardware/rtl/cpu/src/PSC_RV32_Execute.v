...
module PSC_RV32_Execute #(
    ...
    // IDLE is reached before the registered execute_ready pulse updates PC.
    // Accept an interrupt only after that instruction has fully retired;
    // otherwise its pending PC update skips the first trap instruction and
    // mepc still points at the instruction that has already completed.
    assign execute_idle = (execute_state == IDLE) &&
                          (execute_state_d == IDLE) && !execute_ready;
