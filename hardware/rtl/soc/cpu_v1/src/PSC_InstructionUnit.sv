...
// Coordinate pipeline stages, hazards, memory, commit and traps.
module PSC_InstructionUnit (
    ...
    // ID/ISSUE holds decoded control and PC.
    ...
    // ISSUE/EX captures operands after hazard checking.
    typedef struct packed {
        logic valid;
        dec_ctrl_t   ctrl;
        logic [31:0] pc;
    ...
    // EX/MEM holds the ALU/address result, store data and memory control.
    typedef struct packed {
        logic valid;
        dec_ctrl_t   ctrl;
        logic [31:0] pc;
    ...
    // MEM/WB and commit carry the final register write value.
    typedef struct packed {
        logic valid;
        dec_ctrl_t   ctrl;
        logic [31:0] pc;
    ...
    // Serialize instructions that affect control flow, privilege or ordering.
    function automatic logic serializing_instruction(input dec_ctrl_t ctrl);
        serializing_instruction =
            ((ctrl.is_load || ctrl.is_store) && !ctrl.pipeline_type) ||
            (ctrl.pc_sel != 2'b00)           ||
            ctrl.csr_wr                     ||
            ctrl.is_fence                   ||
            ctrl.is_fence_i                 ||
    ...
    // Ordinary branches/jumps may redirect before architectural commit.
    ...
    // Select byte/halfword lanes and sign/zero extend the returned load word.
    ...
    // The youngest matching producer wins; stall until its value is available.
    always_comb begin
        ...
        if (id_issue.ctrl.use_rs1 && (id_issue.ctrl.r_addr1 != 5'd0)) begin
            if (issue_ex.valid && issue_ex.ctrl.rf_wen &&
                ...
                // Wait for MEM/WB to keep the EX ALU off the operand-selection path.
                raw_hazard_rs1   = 1'b1;
                ...
                // Wait for MEM/WB to keep load formatting off the operand-selection path.
                raw_hazard_rs1   = 1'b1;
                ...
                // Wait one cycle for the commit write to reach the register file.
                raw_hazard_rs1   = 1'b1;
    ...
    // Stage handshakes hold EX/MEM and backpressure issue until memory completes.
    ...
    // Stores and control transfers may consume forwarded older ALU results.
    ...
    // Drain admitted instructions before taking a precise timer interrupt.
    ...
    // Branches already redirected from MEM/WB must not redirect again at commit.
    ...
    // Redirect from MEM/WB; retain architectural write-back and CSR effects at commit.
    ...
    // Discard prefetched/decoded instructions on redirects and traps.
    ...
    // MEM/WB already registers the actual MEM-stage branch result. Never
    // classify a simultaneous exception/privilege redirect as a prediction hit.
    assign branch_resolve_valid = mem_wb.valid &&
        early_redirect_instruction(mem_wb.ctrl) &&
        !d_pf && !i_pf && !timer_irq_take && !cpu_trap &&
        !(commit.valid && (commit.ctrl.is_fence_i || commit.ctrl.is_sfence_vma ||
          commit.ctrl.is_ecall || commit.ctrl.is_mret || commit.ctrl.is_sret ||
          commit.ctrl.raise_illegal_instruction));
    assign branch_resolve_taken  = mem_wb.branch_taken;
    assign branch_resolve_pc     = mem_wb.pc;
    assign branch_resolve_target = {mem_wb.alu_data[31:1], 1'b0};

    // Publish the CSR snapshot one edge after the commit write.
    ...
    // ID/ISSUE accepts decoded instructions and discards them on flush.
    always_ff @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
            id_issue  <= '0;
    ...
    // ISSUE/EX captures operands only on successful issue.
    always_ff @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
            issue_ex  <= '0;
    ...
    // EX/MEM captures completed EX results and holds them during memory waits.
    always_ff @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
            ex_mem    <= '0;
        end else if (cpu_stop || d_pf) begin
    ...
    // MEM/WB selects ALU, formatted load, PC+4 or CSR write-back data.
    always_ff @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
            mem_wb    <= '0;
        end else if (cpu_stop || d_pf) begin
    ...
    // Commit retires MEM/WB instructions in order.
    always_ff @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
            commit    <= '0;
        end else if (cpu_stop || d_pf) begin
    ...
    // Architectural PC advances at commit or redirects on branches/traps.
    PSC_PC u_PSC_PC (
        .clock             (clock),
        .reset_n           (reset_n),
        .cpu_stop          (cpu_stop),
