...
module PSC_InstructionUnit #(
    ...
    assign dispatch_blocked =
        (rob[0].valid && rob[0].serializing) ||
        (rob[1].valid && rob[1].serializing);
    ...
    // 実行ユニット種別に応じて候補を分離し、分岐・CSRなどのserializing命令はROB先頭に
    ...
    // IQ sends only register/register RV32M operations to this lane.
    // Do not retain or mux PC/immediate operands on the divider input path.
    always_comb begin
        md_execute_ctrl = md_active_ctrl;
        md_execute_ctrl.op1sel = 1'b0;
        md_execute_ctrl.op2sel = 1'b0;
    end
    ...
    // Address generation is not load data. Never wake a load consumer (or
    // mark its PRF ready) until the MEM response has been formatted.
    ...
    // The ROB result is also the load MEM/WB register. Format on response,
    // then use the registered value for PRF/bypass/IQ wake-up at retirement.
    // This keeps byte selection/sign extension off the forwarding mux path
    // without adding a data register or delaying architectural retirement.
    assign load_wb_valid = commit_fire && rob[rob_head].ctrl.is_load && !d_pf;
    assign load_wb_value = rob[rob_head].result;

    // Load/store EX uses the ordinary IQ/ALU lane, including tag wake-up and
    // dispatch bypass. Address/data live in the existing ROB entry (EX/MEM).
    // Only the MEM request waits for the head, so younger independent EX can
    // overlap a cache miss without speculative memory side effects.
    // Loads and stores issue memory requests only at the ROB head.  A store cannot modify
    ...
    always_ff @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
            ...
            // Invalidate entries; their payload is replaced at dispatch.
            // Do not fan stop/fault out to every operand/result reset mux.
            rob_head  <= '0;
            rob_tail  <= '0;
            rob_count <= '0;
            alu_active <= 1'b0;
            ...
            if (dispatch_fire) begin
                ...
                // Data is written by EX/MEM before completed/address_ready.
                // Reallocation only clears validity, not the wide payload.
                rob[rob_tail].completed       <= 1'b0;
                rob[rob_tail].address_ready   <= 1'b0;
                // Decode once before the ROB/IQ issue-selection timing path.
                rob[rob_tail].serializing     <= serializing_instruction(decode_stage_ctrl);
            ...
            if (alu_select_valid && !alu_active) begin
                ...
                alu_active_dest_valid   <=
                    ...
                    !rob[iq[alu_select_idx].rob_tag].ctrl.wb_sel[0];
            ...
            if (load_response_valid) begin
                rob[rob_head].result <= load_result(load_read_data,
