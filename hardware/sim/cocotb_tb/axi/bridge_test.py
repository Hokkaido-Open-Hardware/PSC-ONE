"""Shared bridge scoreboard: pulse requests, round-robin and AXI stalls.
...
class Bench:
    def __init__(self, dut):
        self.dut = dut
        self.beats = len(dut.d_write_data) // 32
        self.line_bytes = self.beats * 4
    ...
    async def tick(self, reset=False):
        ...
        if not reset:
            ...
            if self.active:
                ...
                if write and self.get('m_axi_wvalid') and self.get('m_axi_wready'):
                    ...
                    assert data == (self.requests[owner][2] >> (index * 32)) & 0xffffffff
        ...
        if not reset:
            ...
            if finished:
                ...
                if not write:
                    expected = sum(self.word(address + n * 4) << (32 * n) for n in range(self.beats))
...
async def shared_bridge_regression(dut):
    ...
    await b.drain()
    b.request('d', True, 0x2007, 0x112233445566778899AABBCCDDEEFF00)
    await b.drain()
    for owner in ('d', 'i'):
