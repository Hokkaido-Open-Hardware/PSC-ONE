...
async def matrix_cycles_and_transactions(dut):
    ...
    for x,y,mode,delay in [(4,4,0,1),(4,4,1,1),(8,4,1,1),(4,8,0,5),(12,20,0,5),(20,12,1,5)]:
        ...
        b = [[rng.choice(values) for _ in range(y)] for _ in range(x)]
        memory = {0x12000+r*x+k:a[r][k]&255 for r in range(y) for k in range(x)}
        memory.update({0x24000+k*y+c:b[k][c]&255 for k in range(x) for c in range(y)})
        expected = {0x48000+(r*y+c)*4:sum(a[r][k]*b[k][c] for k in range(x))&0xffffffff
