...
module PSC_RV32ISP_InstructionEngine (
	...
	wire execute_valid;
	wire [129:0] execute_ctrl;
	wire [31:0] execute_reg_data_1;
	wire [31:0] execute_reg_data_2;
	wire [31:0] execute_alu_data;
	wire execute_done;
...
module PSC_InstructionUnit (
	...
	output wire [129:0] execute_ctrl;
	output wire [31:0] execute_reg_data_1;
	output wire [31:0] execute_reg_data_2;
	input wire [31:0] execute_alu_data;
	input wire execute_done;
	...
	reg [259:0] mem_wb;
	reg [259:0] commit;
	wire [31:0] regfile_rdata_1;
	wire [31:0] regfile_rdata_2;
	wire regfile_wen;
	...
	reg raw_hazard_rs1;
	reg raw_hazard_rs2;
	reg [31:0] issue_reg_data_1;
	reg [31:0] issue_reg_data_2;
	reg [2:0] forward_sel_rs1;
	reg [2:0] forward_sel_rs2;
	reg issue_ex_forwardable;
	reg ex_mem_forwardable;
	reg mem_wb_forwardable;
	reg [31:0] issue_ex_forward_data;
	reg [31:0] ex_mem_forward_data;
	wire backend_serial;
	wire backend_empty;
	wire pipeline_empty;
	wire issue_serial;
	wire issue_forwarding_consumer;
	wire early_branch_valid;
	wire commit_branch_redirected;
	wire pc_update_valid;
	wire [129:0] pc_update_ctrl;
	wire [31:0] pc_update_alu_data;
	wire pc_update_branch_taken;
	...
	function automatic early_redirect_instruction;
		input reg [129:0] ctrl;
		early_redirect_instruction = ((((ctrl[68-:2] != 2'b00) && !ctrl[8]) && !ctrl[9]) && !ctrl[10]) && !ctrl[0];
	endfunction
	...
	assign regfile_wen = (commit[259] && commit[200]) && (commit[248-:5] != 5'd0);
	assign regfile_wdata = (commit[199-:2] == 2'b11 ? csr_rdata : commit[31-:32]);
	PSC_Register u_regfile(
		...
		.r_addr1(id_issue[161-:5]),
		.r_addr2(id_issue[156-:5]),
	...
	always @(*) begin
		if (_sv2v_0)
			;
		issue_ex_forwardable = execute_done && ((issue_ex[166-:2] == 2'b00) || (issue_ex[166-:2] == 2'b10));
		issue_ex_forward_data = (issue_ex[166-:2] == 2'b10 ? issue_ex[95-:32] + 32'd4 : execute_alu_data);
		ex_mem_forwardable = ((ex_mem[198-:2] == 2'b00) || (ex_mem[198-:2] == 2'b10)) || (((ex_mem[198-:2] == 2'b01) && ex_mem[135]) && load_done);
		case (ex_mem[198-:2])
			2'b01: ex_mem_forward_data = load_result(load_read_data, ex_mem[1:0], ex_mem[205-:3]);
			2'b10: ex_mem_forward_data = ex_mem[127-:32] + 32'd4;
			default: ex_mem_forward_data = ex_mem[31-:32];
		endcase
		mem_wb_forwardable = mem_wb[199-:2] != 2'b11;
		issue_reg_data_1 = regfile_rdata_1;
		raw_hazard_rs1 = 1'b0;
		forward_sel_rs1 = 3'd0;
		if (id_issue[37] && (id_issue[161-:5] != 5'd0)) begin
			if (((issue_ex[226] && issue_ex[167]) && (issue_ex[215-:5] != 5'd0)) && (id_issue[161-:5] == issue_ex[215-:5])) begin
				issue_reg_data_1 = issue_ex_forward_data;
				raw_hazard_rs1 = !issue_ex_forwardable;
				forward_sel_rs1 = 3'd1;
			end
			else if (((ex_mem[258] && ex_mem[199]) && (ex_mem[247-:5] != 5'd0)) && (id_issue[161-:5] == ex_mem[247-:5])) begin
				issue_reg_data_1 = ex_mem_forward_data;
				raw_hazard_rs1 = !ex_mem_forwardable;
				forward_sel_rs1 = 3'd2;
			end
			else if (((mem_wb[259] && mem_wb[200]) && (mem_wb[248-:5] != 5'd0)) && (id_issue[161-:5] == mem_wb[248-:5])) begin
				issue_reg_data_1 = mem_wb[31-:32];
				raw_hazard_rs1 = !mem_wb_forwardable;
				forward_sel_rs1 = 3'd3;
			end
			else if (((commit[259] && commit[200]) && (commit[248-:5] != 5'd0)) && (id_issue[161-:5] == commit[248-:5])) begin
				raw_hazard_rs1 = 1'b1;
		...
		if (id_issue[36] && (id_issue[156-:5] != 5'd0)) begin
			if (((issue_ex[226] && issue_ex[167]) && (issue_ex[215-:5] != 5'd0)) && (id_issue[156-:5] == issue_ex[215-:5])) begin
				issue_reg_data_2 = issue_ex_forward_data;
				raw_hazard_rs2 = !issue_ex_forwardable;
				forward_sel_rs2 = 3'd1;
			end
			else if (((ex_mem[258] && ex_mem[199]) && (ex_mem[247-:5] != 5'd0)) && (id_issue[156-:5] == ex_mem[247-:5])) begin
				issue_reg_data_2 = ex_mem_forward_data;
				raw_hazard_rs2 = !ex_mem_forwardable;
				forward_sel_rs2 = 3'd2;
			end
			else if (((mem_wb[259] && mem_wb[200]) && (mem_wb[248-:5] != 5'd0)) && (id_issue[156-:5] == mem_wb[248-:5])) begin
				issue_reg_data_2 = mem_wb[31-:32];
				raw_hazard_rs2 = !mem_wb_forwardable;
				forward_sel_rs2 = 3'd3;
			end
			else if (((commit[259] && commit[200]) && (commit[248-:5] != 5'd0)) && (id_issue[156-:5] == commit[248-:5])) begin
				raw_hazard_rs2 = 1'b1;
	...
	assign memory_complete = (ex_mem[135] ? load_done : (ex_mem[134] ? store_done : 1'b1));
	assign mem_wb_ready = 1'b1;
	assign mem_fire = (ex_mem[258] && memory_complete) && mem_wb_ready;
	assign ex_mem_ready = !ex_mem[258] || mem_fire;
	assign execute_valid = issue_ex[226] && ex_mem_ready;
	assign ex_fire = (issue_ex[226] && execute_done) && ex_mem_ready;
	assign issue_ex_ready = !issue_ex[226] || ex_fire;
	assign backend_empty = ((!issue_ex[226] && !ex_mem[258]) && !mem_wb[259]) && !commit[259];
	assign pipeline_empty = !id_issue[162] && backend_empty;
	assign backend_serial = (((issue_ex[226] && serializing_instruction(issue_ex[225-:130])) || (ex_mem[258] && serializing_instruction(ex_mem[257-:130]))) || (mem_wb[259] && serializing_instruction(mem_wb[258-:130]))) || (commit[259] && serializing_instruction(commit[258-:130]));
	assign issue_serial = serializing_instruction(id_issue[161-:130]);
	assign issue_forwarding_consumer = id_issue[38] || (id_issue[100-:2] != 2'b00);
	assign id_issue_ready = !id_issue[162] || issue_fire;
	assign decode_enb = ((fifo_req_ready && id_issue_ready) && !cpu_stop) && !cpu_trap;
	assign decode_fire = (decode_done && fifo_req_ready) && id_issue_ready;
	assign issue_fire = (((id_issue[162] && issue_ex_ready) && !raw_hazard) && !backend_serial) && ((!issue_serial || backend_empty) || issue_forwarding_consumer);
	assign fifo_read_valid = decode_fire;
	assign execute_ctrl = (issue_ex[226] ? issue_ex[225-:130] : {130 {1'sb0}});
	assign execute_reg_data_1 = issue_ex[63-:32];
	assign execute_reg_data_2 = issue_ex[31-:32];
	assign memory_ctrl = (ex_mem[258] ? ex_mem[257-:130] : {130 {1'sb0}});
	assign memory_alu_data = ex_mem[31-:32];
	assign memory_reg_data_1 = ex_mem[95-:32];
	assign memory_reg_data_2 = ex_mem[63-:32];
	assign memory_pc = ex_mem[127-:32];
	assign load_valid = ex_mem[258] && ex_mem[135];
	assign store_valid = ex_mem[258] && ex_mem[134];
	assign branch_taken_now = branch_exec(ex_mem[196-:2], ex_mem[205-:3], ex_mem[95-:32], ex_mem[63-:32]);
	assign commit_ctrl = (commit[259] ? commit[258-:130] : {130 {1'sb0}});
	assign commit_alu_data = commit[64-:32];
	assign commit_branch_taken = commit[259] && commit[32];
	assign csr_reg_data_1 = commit[96-:32];
	assign csr_enb = commit[259];
	assign commit_branch_redirected = (commit[259] && commit[32]) && early_redirect_instruction(commit[258-:130]);
	assign execute_task_busy = (((id_issue[162] || issue_ex[226]) || ex_mem[258]) || mem_wb[259]) || (commit[259] && !commit_branch_redirected);
	assign execute_task_done = commit[259];
	assign early_branch_valid = (mem_wb[259] && mem_wb[32]) && early_redirect_instruction(mem_wb[258-:130]);
	assign pc_update_valid = early_branch_valid || (commit[259] && !commit_branch_redirected);
	assign pc_update_ctrl = (early_branch_valid ? mem_wb[258-:130] : commit_ctrl);
	assign pc_update_alu_data = (early_branch_valid ? mem_wb[64-:32] : commit_alu_data);
	assign pc_update_branch_taken = early_branch_valid || ((commit[259] && commit[32]) && !commit_branch_redirected);
	assign fifo_flush = ((d_pf || i_pf) || early_branch_valid) || (commit[259] && ((((((commit[162] || commit[161]) || commit[137]) || commit[138]) || commit[139]) || commit[129]) || cpu_trap));
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			csr_valid <= 1'b0;
		else
			csr_valid <= commit[259];
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			id_issue <= 1'sb0;
		else if (cpu_stop || fifo_flush)
			id_issue <= 1'sb0;
		else if (decode_fire) begin
			id_issue[162] <= 1'b1;
			id_issue[161-:130] <= decoded_ctrl;
			id_issue[31-:32] <= pc_now;
		end
		else if (issue_fire)
			id_issue[162] <= 1'b0;
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			issue_ex <= 1'sb0;
		else if (cpu_stop || d_pf)
			issue_ex <= 1'sb0;
		else if (issue_fire) begin
			issue_ex[226] <= 1'b1;
			issue_ex[225-:130] <= id_issue[161-:130];
			issue_ex[95-:32] <= id_issue[31-:32];
			issue_ex[63-:32] <= issue_reg_data_1;
			issue_ex[31-:32] <= issue_reg_data_2;
		end
		else if (ex_fire)
			issue_ex[226] <= 1'b0;
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			...
			ex_mem[258] <= 1'b1;
			ex_mem[257-:130] <= issue_ex[225-:130];
			ex_mem[127-:32] <= issue_ex[95-:32];
			ex_mem[95-:32] <= issue_ex[63-:32];
			ex_mem[63-:32] <= issue_ex[31-:32];
			ex_mem[31-:32] <= execute_alu_data;
		end
		else if (mem_fire)
			ex_mem[258] <= 1'b0;
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			...
			mem_wb[259] <= 1'b1;
			mem_wb[258-:130] <= ex_mem[257-:130];
			mem_wb[128-:32] <= ex_mem[127-:32];
			mem_wb[96-:32] <= ex_mem[95-:32];
			mem_wb[64-:32] <= ex_mem[31-:32];
			mem_wb[32] <= branch_taken_now;
			case (ex_mem[198-:2])
				2'b00: mem_wb[31-:32] <= ex_mem[31-:32];
				2'b01: mem_wb[31-:32] <= load_result(load_read_data, ex_mem[1:0], ex_mem[205-:3]);
				2'b10: mem_wb[31-:32] <= ex_mem[127-:32] + 32'd4;
				default: mem_wb[31-:32] <= 32'd0;
			endcase
		end
		else
			mem_wb[259] <= 1'b0;
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			...
			commit <= 1'sb0;
		else if (mem_wb[259]) begin
			commit[259] <= 1'b1;
			commit[258-:130] <= mem_wb[258-:130];
			commit[128-:32] <= mem_wb[128-:32];
			commit[96-:32] <= mem_wb[96-:32];
			commit[64-:32] <= mem_wb[64-:32];
			commit[32] <= mem_wb[32];
			commit[31-:32] <= mem_wb[31-:32];
		end
		else
			commit[259] <= 1'b0;
	PSC_PC u_PSC_PC(
...
module PSC_Register (
	...
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			for (i = 0; i < 32; i = i + 1)
				registers[i] <= 32'd0;
		else if ((register_wenb && rf_wen) && (w_addr != 5'd0))
