module PSC_ONE_RV32_core (
	...
	parameter [ADDR_WIDTH - 1:0] MMMIO_BASK_BITS = 32'h1000f00f;
	...
	PSC_RV32_core #(.COUNTER_MMIO_ADDR(32'hf004fff0)) u_core(
		.clock(clock),
		.reset_n(reset_n),
		.cpu_stop(cpu_stop),
	...
	assign d_axi_awid = 1'sb0;
	assign d_axi_awaddr = 1'sb0;
	assign d_axi_awlen = 1'sb0;
	assign d_axi_awsize = 1'sb0;
	assign d_axi_awburst = 1'sb0;
	assign d_axi_awvalid = 1'sb0;
	assign d_axi_wdata = 1'sb0;
	assign d_axi_wstrb = 1'sb0;
	assign d_axi_wlast = 1'sb0;
	assign d_axi_wvalid = 1'sb0;
	assign d_axi_bready = 1'sb0;
	assign d_axi_arid = 1'sb0;
	assign d_axi_araddr = 1'sb0;
	assign d_axi_arlen = 1'sb0;
	assign d_axi_arsize = 1'sb0;
	assign d_axi_arburst = 1'sb0;
	assign d_axi_arvalid = 1'sb0;
	assign d_axi_rready = 1'sb0;
	...
	PSC_RV32_Monitor #(.CLK_FREQ_MHz(80)) u_cpu_monitor(
		.clock(clock),
		.reset_n(reset_n),
...
module PSC_RV32_core (
	...
	assign trap_scause = (ecall_u ? 32'd8 : (ecall_s ? 32'd9 : (decoder_ctrl[0] ? 32'd2 : (i_pf_event ? 32'd12 : (d_pf_event ? (data_fault_misaligned ? (data_fault_is_store ? 32'd6 : 32'd4) : (data_fault_is_store ? 32'd15 : 32'd13)) : 32'd0)))));
	assign set_mtrap = ecall_m || timer_irq_take;
	assign trap_mepc = pc;
	assign trap_mcause = (timer_irq_take ? 32'h80000007 : (ecall_m ? 32'd11 : 32'd0));
	wire set_msip;
	wire clr_msip;
	wire set_mtip;
	wire clr_mtip;
	...
	PSC_RV32_InstructionEngine u_inst_engine(
		...
		.data_fault_misaligned(data_fault_misaligned),
			...
			case (fetch_state)
				4'd0:
					if (fetch_valid && (fetch_wakeup_timer > 16'h0300)) begin
						if (initial_fetch && ((BURST_MODE && (count <= (FIFO_DEPTH - 8))) || (!BURST_MODE && !full)))
							next_state = 4'd2;
						else
							next_state = 4'd1;
					end
				...
				4'd4: begin
					if (BURST_MODE)
						next_pc = {fetch_pc[31:5], 5'b00000} + 32'd32;
					else
...
module Fetch_Fifo (
	...
	output wire [WIDTH - 1:0] out_pc_data;
	output wire full;
	output wire empty;
	output reg [ADDR_BITS:0] count;
	input wire flush;
	(* ram_style = "block", syn_ramstyle = "block_ram" *) reg [WIDTH - 1:0] mem [0:DEPTH - 1];
	(* ram_style = "block", syn_ramstyle = "block_ram" *) reg [WIDTH - 1:0] pc_mem [0:DEPTH - 1];
	reg [ADDR_BITS - 1:0] wptr;
	reg [ADDR_BITS - 1:0] rptr;
	wire [ADDR_BITS - 1:0] rptr_next;
	wire [ADDR_BITS - 1:0] read_addr;
	reg [WIDTH - 1:0] ram_data;
	reg [WIDTH - 1:0] ram_pc_data;
	reg [WIDTH - 1:0] bypass_data;
	reg [WIDTH - 1:0] bypass_pc_data;
	reg bypass;
	wire push;
	wire pop;
	...
	assign rptr_next = (rptr == sv2v_cast_5DD1B_signed(DEPTH - 1) ? {ADDR_BITS {1'sb0}} : rptr + 1'b1);
	...
	assign out_data = (empty ? {WIDTH {1'sb0}} : (bypass ? bypass_data : ram_data));
	assign out_pc_data = (empty ? {WIDTH {1'sb0}} : (bypass ? bypass_pc_data : ram_pc_data));
...
module PSC_RV32_InstructionEngine (
	...
	assign d_mmu_enb = mem_mmu_valid && (memory_ctrl[7] || memory_ctrl[6]);
	...
	assign data_mem_read_valid = d_mmu_mem_valid | mem_read_valid;
	assign data_mem_read_address = (d_mmu_mem_valid ? d_mmu_mem_addr : mem_address);
...
module PSC_InstructionUnit (
	...
	output reg [129:0] md_execute_ctrl;
	...
	output wire execute_task_busy;
	output wire execute_task_done;
	reg [(199 + PHY_TAG_W) + 102:0] rob [0:ROB_DEPTH - 1];
	...
	reg commit_prf_valid;
	reg [4:0] commit_prf_addr;
	reg [31:0] commit_prf_data;
	reg commit_prf_csr;
	reg branch_redirect;
	wire pipeline_empty;
	...
	function automatic serializing_instruction;
		input reg [129:0] ctrl;
		serializing_instruction = ((((((((ctrl[68-:2] != 2'b00) || ctrl[31]) || ctrl[34]) || ctrl[33]) || ctrl[32]) || ctrl[8]) || ctrl[9]) || ctrl[10]) || ctrl[0];
	endfunction
	...
	assign dispatch_blocked = (rob[0][199 + (PHY_TAG_W + 102)] && rob[0][196 + (PHY_TAG_W + 102)]) || (rob[1][199 + (PHY_TAG_W + 102)] && rob[1][196 + (PHY_TAG_W + 102)]);
	assign rob_full = rob_count == ROB_DEPTH;
	assign pipeline_empty = (((rob_count == 0) && !decode_stage_valid) && !alu_active) && !md_active;
	assign timer_irq_request = (timer_irq_ext && csr_state[387]) && csr_state[327];
	assign timer_irq_take = timer_irq_request && pipeline_empty;
	assign dispatch_fire = (((((((((decode_stage_valid && !rob_full) && iq_has_free) && (!dispatch_needs_dest || has_free_phys)) && !dispatch_blocked) && !cpu_stop) && !cpu_trap) && !d_pf) && !i_pf) && !fifo_flush) && !commit_prf_valid;
	assign decode_stage_ready = !decode_stage_valid || dispatch_fire;
	assign decode_enb = (((((((fifo_req_ready && decode_stage_ready) && !decode_done) && !cpu_stop) && !cpu_trap) && !d_pf) && !i_pf) && !fifo_flush) && !timer_irq_request;
	assign decode_capture_fire = (((((((decode_done && fifo_req_ready) && decode_stage_ready) && !cpu_stop) && !cpu_trap) && !d_pf) && !i_pf) && !fifo_flush) && !timer_irq_request;
	...
	always @(*) begin
		...
		iq0_alu_candidate = ((iq0_ready && !alu_active) && !is_mul_div(rob[iq[0][ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))-:((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) >= (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) ? ((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) - (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0))))) + 1 : ((33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) - (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))) + 1)]][163 + (PHY_TAG_W + 102)-:((163 + (PHY_TAG_W + 102)) >= (33 + (PHY_TAG_W + 103)) ? ((163 + (PHY_TAG_W + 102)) - (33 + (PHY_TAG_W + 103))) + 1 : ((33 + (PHY_TAG_W + 103)) - (163 + (PHY_TAG_W + 102))) + 1)])) && (!rob[iq[0][ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))-:((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) >= (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) ? ((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) - (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0))))) + 1 : ((33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) - (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))) + 1)]][196 + (PHY_TAG_W + 102)] || (iq[0][ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))-:((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) >= (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) ? ((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) - (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0))))) + 1 : ((33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) - (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))) + 1)] == rob_head));
		iq1_alu_candidate = ((iq1_ready && !alu_active) && !is_mul_div(rob[iq[1][ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))-:((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) >= (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) ? ((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) - (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0))))) + 1 : ((33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) - (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))) + 1)]][163 + (PHY_TAG_W + 102)-:((163 + (PHY_TAG_W + 102)) >= (33 + (PHY_TAG_W + 103)) ? ((163 + (PHY_TAG_W + 102)) - (33 + (PHY_TAG_W + 103))) + 1 : ((33 + (PHY_TAG_W + 103)) - (163 + (PHY_TAG_W + 102))) + 1)])) && (!rob[iq[1][ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))-:((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) >= (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) ? ((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) - (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0))))) + 1 : ((33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) - (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))) + 1)]][196 + (PHY_TAG_W + 102)] || (iq[1][ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))-:((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) >= (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) ? ((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) - (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0))))) + 1 : ((33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) - (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))) + 1)] == rob_head));
	...
	assign alu_execute_ctrl = alu_active_ctrl;
	assign alu_execute_reg_data_1 = alu_active_src1;
	assign alu_execute_reg_data_2 = alu_active_src2;
	assign md_execute_valid = md_active;
	always @(*) begin
		if (_sv2v_0)
			;
		md_execute_ctrl = md_active_ctrl;
		md_execute_ctrl[74] = 1'b0;
		md_execute_ctrl[73] = 1'b0;
	end
	...
	assign load_response_valid = (load_done && load_valid) && !d_pf;
	assign load_wb_valid = (commit_fire && rob[rob_head][(163 + (PHY_TAG_W + 102)) - 122]) && !d_pf;
	assign load_wb_value = rob[rob_head][102-:32];
	...
	assign load_valid = ((((rob_count != 0) && rob[rob_head][199 + (PHY_TAG_W + 102)]) && rob[rob_head][197 + (PHY_TAG_W + 102)]) && !rob[rob_head][198 + (PHY_TAG_W + 102)]) && rob[rob_head][(163 + (PHY_TAG_W + 102)) - 122];
	assign store_valid = ((((rob_count != 0) && rob[rob_head][199 + (PHY_TAG_W + 102)]) && rob[rob_head][197 + (PHY_TAG_W + 102)]) && !rob[rob_head][198 + (PHY_TAG_W + 102)]) && rob[rob_head][(163 + (PHY_TAG_W + 102)) - 123];
	assign commit_fire = ((rob_count != 0) && rob[rob_head][199 + (PHY_TAG_W + 102)]) && rob[rob_head][198 + (PHY_TAG_W + 102)];
	assign commit_result = (rob[rob_head][(163 + (PHY_TAG_W + 102)) - 59-:2] == 2'b11 ? csr_rdata : rob[rob_head][102-:32]);
	always @(*) begin
		if (_sv2v_0)
			;
		commit_ctrl = 1'sb0;
		commit_ctrl[27-:12] = rob[rob_head][(163 + (PHY_TAG_W + 102)) - 102-:12];
		if (commit_fire)
			commit_ctrl = rob[rob_head][163 + (PHY_TAG_W + 102)-:((163 + (PHY_TAG_W + 102)) >= (33 + (PHY_TAG_W + 103)) ? ((163 + (PHY_TAG_W + 102)) - (33 + (PHY_TAG_W + 103))) + 1 : ((33 + (PHY_TAG_W + 103)) - (163 + (PHY_TAG_W + 102))) + 1)];
	end
	...
	assign fifo_flush = (((d_pf || i_pf) || timer_irq_take) || branch_redirect) || (commit_fire && (((((((rob[rob_head][(163 + (PHY_TAG_W + 102)) - 96] || rob[rob_head][(163 + (PHY_TAG_W + 102)) - 97]) || rob[rob_head][(163 + (PHY_TAG_W + 102)) - 121]) || rob[rob_head][(163 + (PHY_TAG_W + 102)) - 120]) || rob[rob_head][(163 + (PHY_TAG_W + 102)) - 119]) || rob[rob_head][(163 + (PHY_TAG_W + 102)) - 129]) || rob[rob_head][37]) || cpu_trap));
	...
	always @(posedge clock or negedge reset_n)
		if (!reset_n) begin
			...
			commit_prf_valid <= (commit_fire && rob[rob_head][1 + (PHY_TAG_W + 102)]) && !rob[rob_head][37];
			commit_prf_addr <= rob[rob_head][(163 + (PHY_TAG_W + 102)) - 10-:5];
			commit_prf_data <= rob[rob_head][102-:32];
			commit_prf_csr <= rob[rob_head][(163 + (PHY_TAG_W + 102)) - 59-:2] == 2'b11;
		end
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			...
			branch_redirect <= (alu_wb_valid && (alu_active_ctrl[68-:2] != 2'b00)) && branch_exec(alu_active_ctrl[68-:2], alu_active_ctrl[77-:3], alu_active_src1, alu_active_src2);
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			csr_valid <= 1'b0;
		else
	...
	always @(posedge clock or negedge reset_n)
		if (!reset_n) begin
			...
			for (i = 32; i < PRF_DEPTH; i = i + 1)
				free_list[i] <= 1'b1;
			for (i = 0; i < ROB_DEPTH; i = i + 1)
				rob[i][199 + (PHY_TAG_W + 102)] <= 1'b0;
			for (i = 0; i < IQ_DEPTH; i = i + 1)
				iq[i][1 + (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))] <= 1'b0;
		end
			...
			if (commit_fire) begin
				rob[rob_head][199 + (PHY_TAG_W + 102)] <= 1'b0;
			...
			if (dispatch_fire) begin
				rob[rob_tail][199 + (PHY_TAG_W + 102)] <= 1'b1;
				rob[rob_tail][198 + (PHY_TAG_W + 102)] <= 1'b0;
				rob[rob_tail][197 + (PHY_TAG_W + 102)] <= 1'b0;
				rob[rob_tail][196 + (PHY_TAG_W + 102)] <= serializing_instruction(decode_stage_ctrl);
			...
			if (alu_select_valid && !alu_active) begin
				...
				alu_active_dest_valid <= rob[iq[alu_select_idx][ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))-:((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) >= (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) ? ((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) - (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0))))) + 1 : ((33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) - (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))) + 1)]][1 + (PHY_TAG_W + 102)] && !rob[iq[alu_select_idx][ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))-:((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) >= (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) ? ((ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32)))) - (33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0))))) + 1 : ((33 + (PHY_TAG_W + (33 + (PHY_TAG_W + 0)))) - (ROB_TAG_W + (33 + (PHY_TAG_W + (PHY_TAG_W + 32))))) + 1)]][(163 + (PHY_TAG_W + 102)) - 60];
			...
			if (alu_wb_valid) begin
				...
				rob[alu_wb_rob_tag][197 + (PHY_TAG_W + 102)] <= alu_active_ctrl[7] || alu_active_ctrl[6];
				if (!alu_active_ctrl[7] && !alu_active_ctrl[6])
					rob[alu_wb_rob_tag][198 + (PHY_TAG_W + 102)] <= 1'b1;
			end
			...
			if (md_wb_valid) begin
				...
				rob[md_active_rob_tag][198 + (PHY_TAG_W + 102)] <= 1'b1;
				md_active <= 1'b0;
			end
			if (load_response_valid) begin
				rob[rob_head][102-:32] <= load_result(load_read_data, rob[rob_head][72:71], rob[rob_head][(163 + (PHY_TAG_W + 102)) - 52-:3]);
				rob[rob_head][198 + (PHY_TAG_W + 102)] <= 1'b1;
			end
...
module Fetch (
	...
	reg [2:0] burst_count;
	assign fifo_read_pc = (BURST_MODE ? {vaddr[31:5], 5'b00000} + {27'd0, burst_count[2:0], 2'b00} : vaddr);
	reg [3:0] state;
	reg [2:0] burst_start_word;
	assign program_mem_burst_mode = BURST_MODE;
	assign fifo_read_valid = program_mem_read_ready && (!BURST_MODE || (burst_count[2:0] >= burst_start_word));
			...
			case (state)
				...
				4'd3:
					if (program_mem_read_ready) begin
						...
						else if (burst_count[2:0] == burst_start_word)
							opcode <= program_mem_read_data;
						if (BURST_MODE) begin
...
module Decorder (
	...
	wire pipeline_type_w = ((is_R_type_w || is_op_imm_w) || is_load_w) || is_store_w;
			...
			case (state)
				2'd0:
					if (div_start)
						...
						multi_result <= alu_exec(decoder_ctrl[82-:5], operand_1, operand_2);
						state <= 2'd3;
					end
				2'd1:
					if (div_done) begin
...
module Execute_Mul (
	...
	reg [1:0] state;
	reg signed [32:0] multiplicand_q;
	reg signed [32:0] multiplier_q;
	...
	always @(posedge clk or negedge reset_n)
		if (!reset_n) begin
			...
			case (state)
				2'd0:
					if (start) begin
						multiplicand_q <= ((alucon == 2'b01) || (alucon == 2'b10) ? $signed({data_1[31], data_1}) : $signed({1'b0, data_1}));
						multiplier_q <= (alucon == 2'b01 ? $signed({data_2[31], data_2}) : $signed({1'b0, data_2}));
						high_result_q <= alucon != 2'b00;
						state <= 2'd1;
					end
				2'd1: begin
					mul_out <= (high_result_q ? product[63:32] : product[31:0]);
					done <= 1'b1;
					state <= 2'd0;
				end
				default: state <= 2'd0;
...
module Branch (
	...
	output reg pending;
	output wire done;
	output reg misaligned_fault;
	reg mmu_started;
	reg translated;
	reg [31:0] translated_address;
	wire misaligned_address;
	assign misaligned_address = (((decoder_ctrl[77-:3] == 3'b001) || (decoder_ctrl[7] && (decoder_ctrl[77-:3] == 3'b101))) && in_vaddr[0]) || ((decoder_ctrl[77-:3] == 3'b010) && |in_vaddr[1:0]);
	assign done = pending && (decoder_ctrl[6] ? data_mem_write_ready : data_mem_read_ready);
	always @(posedge clock or negedge reset_n)
		if (!reset_n) begin
			pending <= 1'b0;
			mmu_started <= 1'b0;
			...
			if ((cpu_stop || (access_fault || misaligned_fault)) || done) begin
			...
			else if (valid && !pending) begin
				if (misaligned_address)
					misaligned_fault <= 1'b1;
				else if (mode_sv32 && !translated) begin
				...
				else if (data_mem_req_ready) begin
					pending <= 1'b1;
					data_mem_read_valid <= decoder_ctrl[7];
					data_mem_write_valid <= decoder_ctrl[6];
					data_mem_address <= (mode_sv32 ? translated_address : in_vaddr);
					data_mem_write_data <= r_data2;
					if (decoder_ctrl[6] && (in_vaddr == UART_MMIO_ADDR))
...
module PSC_PC (
	...
	assign trap_pc = (!interrupt && trap_deleg_to_s ? csr_state[159-:32] : csr_state[287-:32]);
...
module Csr (
	...
	reg [31:0] csr_write_newv;
	reg csr_write_no_side_effect_rs;
	always @(posedge clock or negedge reset_n)
		if (!reset_n)
			csr_rdata <= 32'd0;
		else
	...
	always @(posedge clock or negedge reset_n)
		if (!reset_n) begin
			...
			csr_mtval <= 32'b00000000000000000000000000000000;
			...
			if (csr_write_pending)
				case (csr_write_addr)
					...
					12'h343: csr_mtval <= csr_write_newv;
