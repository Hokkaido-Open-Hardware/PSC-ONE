...
- `src/api/tflite/tflite_api.h`: C APIと所有権規約。
- `src/api/tflite/tflite_runtime.cc`: prepare/arena/CPU FC/invoke/trace。
- `src/api/tflite/tflite_quant.h`, `tflite_quant.cc`: 公式整数補助関数adapter。
- `src/api/tflite/tflite_demo.h`: 再生成を照合する固定入力・全中間期待値。
- `src/api/tflite/PHASE3.md`: 本報告。
...
- `src/api/tflite/tflite_file.c`: 共有bufferによるload/prepare、シェルdemo、時間計測。
- `src/api/tflite/tflite_inspect.cc/.h`, `README.md`: profile表示とPhase 3への説明更新。
- `src/shell/shell.c`: `tflite_run`とhelp。
- `src/drivers/timer_measure.c`, `timer_api.h`, `syscall.h`, `kernel_syscall.c`, `user.c`, `user.h`:
