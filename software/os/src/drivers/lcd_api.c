...
//
// boot_logo_rle[] : 480x320, lossless runs of 3-bit RGB pixels
//
// bit0 = Red
// bit1 = Green
// bit2 = Blue
...
void lcd_draw_boot_logo(void)
{
...
#if 1
    // Decode runs while streaming to the LCD; no framebuffer allocation.
    ...
    for (uint32_t by = 0; by < 320; by += 1) {
        ...
        for (uint32_t bx = 0; bx < 480; bx += 1) {
            ...
            if (remaining == 0) {
                uint32_t run = boot_logo_rle[run_index++];
                remaining = run >> 3;
                c = run & 7;
            }
            --remaining;
