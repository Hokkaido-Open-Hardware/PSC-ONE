...
static int measure_end(unsigned divisor)
{
    ...
    measure_active = 0;
    return (int)(elapsed / divisor);
}

/* Non-destructive snapshot for profiling inside an active stopwatch.
   Retry a wrap observed between status and count; never borrow a busy timer. */
int timer_measure_read_us(void) {
    if (!measure_active || measure_stopping) return -1;
    uint32_t before, after, pending, pending_after, count;
    do {
        before = measure_us;
        pending = timer_get_status() & TIMER_ST_IRQ_PENDING;
        count = timer_get_count();
        pending_after = timer_get_status() & TIMER_ST_IRQ_PENDING;
        after = measure_us;
    } while (before != after || pending != pending_after);
    return (int)(before + (pending ? MEASURE_PERIOD_US : 0) + MEASURE_RELOAD - count);
}

int timer_measure_end(void) { return measure_end(1000u); }
int timer_measure_end_us(void) { return measure_end(1u); }
