...
#define MACHINE_CONTEXT_SIZE 160
...
_Static_assert(sizeof(struct trap_frame) == 144, "S trap assembly layout mismatch");
...
void kernel_entry(void) {
    __asm__ __volatile__(
        /* sscratch=kernel stack in U, zero while handling an S trap.
           M-mode context preserves sscratch even if it preempts this swap. */
        ...
        "csrr a0, stval\n"
        "sw a0, 4 * 34(sp)\n"
        /* Existing pointer syscalls copy U pages in S-mode. */
        "li a0, (1 << 18)\n"
        "csrs sstatus, a0\n"
        "mv a0, sp\n"
        "call handle_trap\n"
...
{
    uint32_t scause = f->scause;
    uint32_t stval = f->stval;
    uint32_t sepc = f->sepc;
    uint32_t sstatus = f->sstatus;
    ...
    if (!(sstatus & (1u << 8)) && !(scause & 0x80000000u) && elf_is_running()) {
        s_printf("ELF: fault cause=%x pc=%x value=%x\n", scause, sepc, stval);
        elf_finish(f, PSC_ELF_FAULT, (int)scause);
        return;
    }
