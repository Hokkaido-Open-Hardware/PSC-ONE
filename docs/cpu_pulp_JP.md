...
[English](cpu_pulp.md) · [ドキュメント一覧](README.md) · [PSC-ONE](../README_JP.md)

CPU v1が実装する2つのドット積命令の仕様と、2026-09-21の実装・検証記録をまとめます。
仕様、実装時の観測値、将来案を分けて記載します。ここにあるPASS・性能・資源値は
保存された過去の検証記録であり、文書更新時に再実行した結果ではありません。

<!-- contents -->
- [1. 目的](#1-目的)
- [2. 実装対象](#2-実装対象)
- [3. `cv.dotup.h`](#3-cvdotuph)
- [4. `cv.dotsp.b`（signed byte SIMD）](#4-cvdotspbsigned-byte-simd)
- [5. RTL実装要件と共通動作](#5-rtl実装要件と共通動作)
- [6. リファレンスモデル](#6-リファレンスモデル)
- [7. 検証項目](#7-検証項目)
- [8. ソフトウェア側の扱い](#8-ソフトウェア側の扱い)
- [9. CPU v1の実装とSDOTSP撤去記録](#9-cpu-v1の実装とsdotsp撤去記録)
- [10. 撤去後の機能検証記録（2026-09-21）](#10-撤去後の機能検証記録2026-09-21)
- [11. NN性能比較（記録値）](#11-nn性能比較記録値)
- [12. SDOTSP撤去前後の合成・配置配線](#12-sdotsp撤去前後の合成配置配線)
- [13. 現行PULP拡張の費用対効果](#13-現行pulp拡張の費用対効果)
- [14. 将来拡張](#14-将来拡張)
- [15. 互換性と出典](#15-互換性と出典)
<!-- /contents -->
...
PSC-ONEのCPU v1は、PULP/CORE-V ISAのSIMDドット積命令のうち2命令を実装する。
...
DOTUP.Hは以下のとおり。DOTSP.Bは各byteを−128〜127へ符号変換して積和する。
...
機能の有無はビルド構成で管理する。上記マクロとPSC固有feature CSRは提案であり、
既存の実装済みインターフェースを意味しない。

実際の利用例は[cv_pulp_test1.cpp](../hardware/sim/cpp/cv_pulp_test1.cpp)、
[cv_pulp_test2.cpp](../hardware/sim/cpp/cv_pulp_test2.cpp)、
[TFLite PULPバックエンド](../software/os/third_party/tflite/README.md)を参照。

## 9. CPU v1の実装とSDOTSP撤去記録

以下は撤去時の構成の記録。最新の内部ステートは
[Decorder.sv](../hardware/rtl/soc/cpu_v1/src/Decorder.sv)、
[Execute.sv](../hardware/rtl/soc/cpu_v1/src/Execute.sv)、
[Execute_Mul.sv](../hardware/rtl/soc/cpu_v1/src/Execute_Mul.sv)を参照。
...
以下は当時の検証で、Execute受理の立上りをt=0としたdoneまでの遅延（下流stallなし）。
現行RTLの内部ステートや遅延を再測定した値ではない。
...
## 10. 撤去後の機能検証記録（2026-09-21）

再現手順は[符号付きbyte SIMD回帰](../hardware/sim/tests/v1_cv_signed_byte/README.md)。
DOTUP.H追加時点の記録は[初期検証記録](cpu_pulp_validation.md)を参照。
同記録の「有効1命令／illegal 1,023」はDOTSP.B追加前の値である。
...
以下は `PSC-ONE/hardware/sim` から実行する。
...
高速化率3.627倍、削減率72.43%。**撤去後に測定した2方式バイナリでの実測**は次のとおり。
